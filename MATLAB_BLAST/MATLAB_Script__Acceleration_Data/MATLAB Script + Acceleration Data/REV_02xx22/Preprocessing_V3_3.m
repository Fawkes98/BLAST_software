...
% Version 3.3 (adding motor spec vs 
...
% braking
brake_trig = -0.02;      % (rad/s^2) slope to trigger brake {limit >= -.005}
...
Data_Out = [t' theta_proc' omega_proc' brakes'];
...
% parameters, add braking section as well for 3rd line



% motionProfile = zeros(length(Data_Out),3);
%     for i = 1:length(Data_Out)
%         for j = 2:size(motionProfile, 2)
%             motionProfile(i, j) = Data_Out(i, j);
%         end
%     end
