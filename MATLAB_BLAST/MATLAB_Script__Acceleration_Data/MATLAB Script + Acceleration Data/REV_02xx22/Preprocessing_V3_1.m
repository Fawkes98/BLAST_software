...
%                                                - (CALC POS BASED ON SMOOTHED PROFILE)
% - Brake Flagging:
%   - find braking points
...
rad_pos = [0]; % initial condition
for i = 2:length(omega_raw)
    rad_pos(i) = rad_pos(i-1) + (omega_raw(i-1)*dt);
end
rad_pos = rad_pos/(2*pi); % change in terms of full rotations (ie 2*pi rads)
...
hold on
grid on
plot(t, rad_pos, "k", "linewidth", 2)
%plot(t, omega_proc, "r", "linewidth", 2)
%plot(t(brakes == 1), omega_proc(brakes == 1), ".b", "markersize", 10) 
title("Angular Position")
xlabel("Time (s)")
ylabel("Postion (2/pi rads)")
legend("Original", "Processed", "location", "north")

% %sanity test
% figure(6)
% hold on; grid on;
% plot(t, pos_raw, 'k', 'linewidth', 2);
...
% data format = [time (s), angular speed (rad/s), brake flag (0/1)]
